...
const methodologyItems: Array<[LucideIcon,string,string]> = [[Users,"Custom student image dataset","Planned minimum of 50 images per student"],[Eye,"Visual variation","Different lighting conditions and poses"],[Video,"Behavior recordings","Planned classroom video for analysis"],[Layers3,"Supplementary sources","DAiSEE and COCO public datasets"],[Cpu,"Pre-processing","Cleaning, normalization, and preparation"],[Sparkles,"Augmentation","Variation to improve robustness and balance"]];
function Methodology(){return <section className={sectionClass}><div className="grid gap-12 lg:grid-cols-[.85fr_1.15fr]"><SectionHeading eyebrow="11 / Data & Methodology" title="A planned evidence pipeline" copy="Dataset collection and model evaluation form part of the proposed work. No dataset is represented here as already collected."/><div className="reveal grid gap-3 sm:grid-cols-2">{methodologyItems.map(([Icon,t,d])=><div key={t} className="rounded-md border border-border bg-card p-5"><Icon size={18} className="text-primary"/><p className="mt-4 font-display text-sm font-semibold">{t}</p><p className="mt-2 text-xs leading-5 text-muted-foreground">{d}</p></div>)}</div></div></section>}

const privacyItems: Array<[LucideIcon,string,string]> = [[LockKeyhole,"Institutional privacy","Facial data handled according to institutional privacy policies."],[Database,"Local storage","Data intended to be stored locally and not shared externally."],[Eye,"Lighting variability","Diverse training data planned to address changing conditions."],[Cpu,"Processing capacity","GPU acceleration considered for real-time processing."],[Layers3,"Dataset imbalance","Data augmentation planned to improve class balance."]];
function Privacy(){return <section id="privacy" className="border-y border-border bg-panel/30"><div className={sectionClass}><div className="reveal rounded-lg border border-primary/25 bg-primary/5 p-7 sm:p-10"><div className="grid gap-10 lg:grid-cols-[.7fr_1.3fr]"><div><span className="grid size-12 place-items-center rounded-md border border-primary/30 bg-primary/10 text-primary"><ShieldCheck size={25}/></span><p className="mt-6 font-display text-xs font-bold uppercase text-primary">12 / Privacy & Risk Management</p><h2 className="mt-3 font-display text-3xl font-semibold">Responsible by design</h2><p className="mt-4 text-sm leading-6 text-muted-foreground">The prototype approach prioritizes controlled access, local handling, and technical safeguards.</p></div><div className="grid gap-px overflow-hidden rounded-md border border-border bg-border sm:grid-cols-2">{privacyItems.map(([Icon,t,d])=><div key={t} className="bg-background p-5"><Icon size={17} className="text-primary"/><p className="mt-3 text-sm font-semibold">{t}</p><p className="mt-2 text-xs leading-5 text-muted-foreground">{d}</p></div>)}</div></div></div></div></section>}
